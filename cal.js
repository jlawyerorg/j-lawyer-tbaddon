...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    console.log(
      "Kalender-Name:",
      calendarSelect.options[calendarSelect.selectedIndex].text,
    );
    ...
    logActivity("Kalendereintrag erstellt", {
      calAssignee,
      calCalendar,
      calSummary,
      calType,
  ...
  try {
    ...
    document.querySelectorAll(".resultItem").forEach((item) => {
      item.addEventListener("click", async function () {
        ...
        customizableLabel.textContent =
          currentSelectedCase.fileNumber +
          ": " +
          currentSelectedCase.name +
          " (" +
          caseMetaData.reason +
          " - " +
          caseMetaData.lawyer +
      ...
      item.addEventListener("mouseover", async function () {
        ...
        const metaData = await getCaseMetaData(
          caseId,
          loginData.username,
          loginData.password,
          loginData.serverAddress,
...
// Funktion zum Ermitteln der Länge der längsten aufeinanderfolgenden Übereinstimmung (nicht mehr benötigt, aber für Kompatibilität beibehalten)
...
async function setDueDate(
  ...
  const headers = new Headers();
  const loginBase64Encoded = btoa(
    unescape(encodeURIComponent(loginData.username + ":" + loginData.password)),
  );
  headers.append("Authorization", "Basic " + loginBase64Encoded);
  // headers.append('Authorization', 'Basic ' + btoa('' + username + ':' + password + ''));
  headers.append("Content-Type", "application/json");
  ...
  const url =
    loginData.serverAddress + "/j-lawyer-io/rest/v6/cases/duedate/create";
