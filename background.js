...
browser.messageDisplayAction.onClicked.addListener(async (tab) => {
  ...
  const width = savedSize.popupWindowSize?.width || 560;
  const height = savedSize.popupWindowSize?.height || 800;

  // Öffne popup.html als eigenständiges Fenster mit Message-ID als URL-Parameter
  const popupUrl = browser.runtime.getURL(`popup.html?messageId=${message.id}`);
...
// Setze popupWindowId zurück wenn das Fenster geschlossen wird
...
async function sendEmailToServer(
  ...
  if (customFilename) {
    ...
    fileName =
      dateString + "_" + messageData.author + messageData.subject + ".eml";
    fileName = fileName.replace(/[\/\\:*?"<>|@]/g, "_");
  }
  ...
  try {
    fileNamesArray = await getFilesInCase(
      caseId,
      username,
      password,
      serverAddress,
  ...
  if (fileNamesArray.includes(fileName)) {
    ...
    try {
      ...
      if (moveToTrashResult.moveToTrash) {
        ...
        await logActivity(
          "sendEmailToServer",
          "Email in Papierkorb verschoben",
        );
      }
...
async function sendOnlyMessageToServer(
  caseId,
  username,
  password,
  serverAddress,
...
async function sendAttachmentsToServer(
  caseId,
  username,
  password,
  serverAddress,
  ...
  if (attachments.length === 0) {
    ...
    browser.runtime.sendMessage({
      type: "error",
      content: "Keine Attachments in der Nachricht vorhanden.",
    });
    return;
  }
  ...
  try {
    ...
    browser.runtime.sendMessage({
      type: "error",
      content: "Fehler beim Abrufen der vorhandenen Dateien",
    });
    return;
  }
  ...
  for (let att of attachments) {
    attachmentCounter++;
    console.log(
      `Verarbeite Attachment ${attachmentCounter}/${attachments.length}: ${att.name}`,
    );
    ...
    try {
      ...
      while (
        fileNamesArray.includes(finalFileName) ||
      ...
      if (finalFileName !== fileName) {
        console.log(
          `Dateiname geändert von "${fileName}" zu "${finalFileName}" wegen Duplikat`,
        );
      }

      console.log(
        `CasefolderID für Attachment "${finalFileName}": ${selectedCaseFolderID}`,
      );
      ...
      console.error(
        `Fehler beim Verarbeiten von Attachment "${att.name}":`,
        error,
      );
      errors.push({
        fileName: att.name,
  ...
  // Ergebnis-Summary
  console.log(
    `Attachment-Verarbeitung abgeschlossen: ${processedFiles.length}/${attachments.length} erfolgreich`,
  );
  ...
  if (processedFiles.length > 0) {
    browser.runtime.sendMessage({
      type: "success",
      content: `${processedFiles.length} Attachment(s) erfolgreich hochgeladen`,
    });
  ...
  await sendAllMimePartsToServer(
    caseId,
    username,
    password,
    serverAddress,
...
async function sendEmailToServerAfterSend(
  caseIdToSaveToAfterSend,
  selectedCaseFolderIDAfterSend,
  username,
  password,
  ...
  console.log(
    "selectedCaseFolderIDAfterSend: " + selectedCaseFolderIDAfterSend,
  );
...
async function getFilesInCase(caseId, username, password, serverAddress) {
  const url =
    serverAddress + "/j-lawyer-io/rest/v1/cases/" + caseId + "/documents";
...
async function getFilesInCaseToDownload(
  caseId,
  username,
  password,
  serverAddress,
...
async function getFileByIdToDownload(
  fileId,
  username,
  password,
  serverAddress,
  ...
  const url =
    serverAddress +
    "/j-lawyer-io/rest/v1/cases/document/" +
    fileId +
    "/content";
...
async function setDocumentTagsAndFolderForAttachments() {
  ...
  try {
    ...
    const headers = new Headers();
    const loginBase64Encoded = btoa(
      unescape(encodeURIComponent(result.username + ":" + result.password)),
    );
    ...
    if (result.selectedTags && result.selectedTags.length > 0) {
      for (let documentTag of result.selectedTags) {
        for (let docInfo of documentIdsToTag) {
          ...
          const url =
            result.serverAddress +
            "/j-lawyer-io/rest/v5/cases/documents/" +
            documentId +
            "/tags";
          ...
          if (!response.ok) {
            throw new Error(
              `Network response was not ok for document ID ${documentId}`,
            );
          }
        }
      }
...
browser.runtime.onMessage.addListener((message) => {
    ...
    return browser.storage.local
      ...
      .then(async (result) => {
        ...
        if (!caseId) {
          return {
            ok: false,
            error: "Keine Akten-ID übergeben",
          };
        }
  ...
  if (
    ...
    browser.storage.local
      ...
      .then(async (result) => {
        const caseId = getCaseIdFromMessage(message);
        console.log("Die übergebene Akten-ID lautet: " + caseId);
  ...
  if (message.type === "overlay-ready") {
    console.log(
      "Background: overlay-ready message received - using storage-based communication",
    );
    return;
  }
  ...
  if (message.type === "load-image") {
    console.log(
      "Background: load-image message received - using storage-based communication",
    );
    return;
  }
  ...
  if (message.type === "image-cropped") {
    console.log(
      "Background: image-cropped message received - using storage-based communication",
    );
    return;
  }
  ...
  if (message.type === "skip-image") {
    console.log(
      "Background: skip-image message received - using storage-based communication",
    );
    return;
  }
  ...
  if (message.type === "finish-editing") {
    console.log(
      "Background: finish-editing message received - using storage-based communication",
    );
    return;
  }
  ...
  if (message.type === "cancel-editing") {
    console.log(
      "Background: cancel-editing message received - using storage-based communication",
    );
    return;
  }
  ...
  if (message.type === "show-finish-options") {
    console.log(
      "Background: show-finish-options message received - using storage-based communication",
    );
    return;
  }
  ...
  if (
    ...
    (async () => {
      ...
      try {
        ...
        const caseIdToSaveToAfterSend = getCaseIdFromMessage(message);

        if (!caseIdToSaveToAfterSend) {
          throw new Error(
            "Keine Akten-ID für das Speichern nach Versand übergeben.",
          );
        }
...
async function createMenuEntries() {
  try {
    ...
    if (emailTemplates.length > 0) {
      ...
      for (const template of emailTemplates) {
        try {
          ...
          console.error(
            "Fehler beim Erstellen eines Vorlagen-Menüeintrags:",
            e,
            template,
          );
        }
    ...
    try {
      ...
      const folderData = await getCaseFolders(
    ...
    function folderHasDocsOrNonEmptyChildren(folder) {
      ...
      if (folder.children && Array.isArray(folder.children)) {
        return folder.children.some(
          (child) => child && folderHasDocsOrNonEmptyChildren(child),
        );
      }
      return false;
    }
    ...
    async function createFolderMenu(folder, parentId) {
      ...
      const docs = (docsByFolder[id] || []).sort((a, b) =>
    ...
    if (documentsInSelectedCase.length > 0) {
      ...
      if (folders && folders.id) {
        ...
        const rootDocs = documentsInSelectedCase
          .filter((doc) => rootFolderIds.includes(doc.folderId))
    ...
    if (!isMenuClickListenerRegistered) {
      browser.menus.onClicked.addListener(async (info, tab) => {
        try {
          if (
            info.menuItemId === "vorlagen-menu" ||
            info.menuItemId === "dokumente-menu"
          ) {
            return;
          }
          ...
          const clickedDocument = currentDocuments.find(
            (document) => `dokument-${document.id}` === info.menuItemId,
          );

          if (clickedTemplate) {
            const clickedTemplateNameEncoded = encodeURIComponent(
            ...
            try {
              ...
              console.error(
                "Fehler beim Abrufen der Daten oder beim Einfügen der Vorlage: ",
                error,
              );
            }
            ...
            let downloadedFile = await getFileByIdToDownload(
              IdOfDocumentToAddToMail,
              loginData.username,
              loginData.password,
              loginData.serverAddress,
            );
            ...
            console.log(
              "Kein passender Menüeintrag gefunden für ID:",
              info.menuItemId,
            );
          }
        } catch (e) {
...
function getTemplateWithPlaceholders(
  ...
  const url =
    serverAddress +
    "/j-lawyer-io/rest/v6/templates/email/" +
    templateName +
    "/" +
    caseId;
  //console.log("URL: " + url);
...
async function extractAttachmentFromRawMessage(messageId, mimePart, partPath) {
  try {
    console.log(
      `Starte Raw-Message Extraktion für Part: ${partPath}, Name: ${mimePart.name || "unbekannt"}`,
    );

    // Raw-Message holen
    const rawMessage = await browser.messages.getRaw(messageId, {
      decrypt: true,
    });
    if (!rawMessage) {
    ...
    if (!targetPart && mimePart.name) {
      ...
      for (let i = 1; i < parts.length - 1; i++) {
        // Erste und letzte sind leer/Epilog
        const part = parts[i];
        if (
...
async function extractSinglePartAttachment(rawMessage, mimePart) {
  try {
    console.log(
      `Versuche Single-Part Extraktion für: ${mimePart.name || "unbekannt"}`,
    );
    ...
    if (rawMessage.includes("Content-Transfer-Encoding: base64")) {
      ...
      if (base64Content) {
        console.log(
          `Single-Part Base64-Inhalt extrahiert: ${base64Content.length} Zeichen`,
        );
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmedLine.toLowerCase().includes("content-transfer-encoding:")) {
        const encodingMatch = trimmedLine.match(
          /content-transfer-encoding:\s*(.+)/i,
        );
        if (encodingMatch) {
    ...
    if (base64Content) {
      console.log(
        `Base64-Inhalt aus MIME-Part extrahiert: ${base64Content.length} Zeichen (Encoding: ${encoding})`,
      );
      return base64Content;
    }
...
async function sendAllMimePartsToServer(
  caseId,
  username,
  password,
  serverAddress,
  ...
  let fullMessage;
  try {
    fullMessage = await browser.messages.getFull(messageData.id, {
      decrypt: true,
    });
    ...
    browser.runtime.sendMessage({
      type: "error",
      content: "Fehler beim Analysieren der E-Mail-Struktur",
    });
    return;
  }
  ...
  try {
    ...
    console.log(
      "Dateinamen für MIME-Parts abgerufen:",
  ...
  async function processMimePart(part, partPath = "") {
    ...
    const hasInlineDisposition =
      part.headers &&
      part.headers["content-disposition"] &&
      part.headers["content-disposition"].some((disp) =>
        disp.toLowerCase().includes("inline"),
      );

    // Prüfen auf Content-Disposition mit Dateinamen (auch ungewöhnliche Formate)
    const hasContentDispositionWithFilename =
      part.headers &&
      part.headers["content-disposition"] &&
      part.headers["content-disposition"].some((disp) =>
    ...
    if (!isRealAttachment || isMainEmailContent) {
      const contentDisposition =
        part.headers && part.headers["content-disposition"]
          ? part.headers["content-disposition"].join("; ")
          : "keine";
      console.log(
        `MIME-Part übersprungen - PartPath: ${partPath}, Name: ${part.name || "ohne"}, ContentType: ${contentType}, Content-Disposition: ${contentDisposition}, Grund: ${
      ...
      const contentDisposition =
        part.headers && part.headers["content-disposition"]
          ...
          : "keine";
      console.log(
        `MIME-Part wird verarbeitet - PartPath: ${partPath}, Name: ${part.name || "ohne"}, ContentType: ${contentType}, Content-Disposition: ${contentDisposition}`,
      );
    }
    ...
    if (isRealAttachment && !isMainEmailContent) {
      ...
      if (part.partName && alreadyProcessedFiles.length > 0) {
        ...
        if (tempAttachmentName) {
          ...
          const alreadyProcessed = alreadyProcessedFiles.some(
            (processedFile) => {
              // Entferne Datum-Prefix und vergleiche Basis-Namen
              const baseName = processedFile
                .replace(/^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}_/, "")
                .replace(/_\d+(\.[^.]+)?$/, "$1");
              return (
                baseName === tempAttachmentName ||
                processedFile.includes(tempAttachmentName)
              );
            },
          );
          ...
          if (alreadyProcessed) {
            console.log(
              `MIME-Part übersprungen - bereits als regulärer Anhang verarbeitet: ${tempAttachmentName} (partName: ${part.partName})`,
            );
      ...
      if (!part.body) {
        console.warn(
          `MIME-Part hat keinen Body, versuche trotzdem zu verarbeiten: PartPath ${partPath}, Name: ${part.name || "ohne"}`,
        );
      }
      mimePartCounter++;
      const partName = part.partName || `${partPath}.${mimePartCounter}`;
      console.log(
        `Verarbeite MIME-Part ${mimePartCounter}: ${part.name} (${partName})`,
      );
      ...
      try {
        ...
        if (part.partName) {
          try {
            ...
            if (attachmentData) {
              ...
              console.log(
                `Standard-API erfolgreich: ${emailContentBase64.length} Zeichen Base64`,
              );
            }
          } catch (apiError) {
            console.warn(
              `Standard-API fehlgeschlagen für ${part.partName}: ${apiError.message}`,
            );
          }
        }
        ...
        if (!emailContentBase64) {
          console.log(
            `Versuche Raw-Message Extraktion für MIME-Part ${mimePartCounter}`,
          );
          try {
            emailContentBase64 = await extractAttachmentFromRawMessage(
              messageData.id,
              part,
              partPath,
            );
            if (emailContentBase64) {
              console.log(
                `Raw-Message Extraktion erfolgreich: ${emailContentBase64.length} Zeichen Base64`,
              );
            }
          } catch (rawError) {
            console.warn(
              `Raw-Message Extraktion fehlgeschlagen: ${rawError.message}`,
            );
          }
        }
        ...
        if (!emailContentBase64 && part.body && part.body.trim() !== "") {
          console.log(
            `Verwende part.body als Fallback für MIME-Part ${mimePartCounter}`,
          );
          try {
            emailContentBase64 = btoa(unescape(encodeURIComponent(part.body)));
            console.log(
              `part.body erfolgreich kodiert: ${emailContentBase64.length} Zeichen Base64`,
            );
          } catch (bodyError) {
            console.warn(
              `part.body Kodierung fehlgeschlagen: ${bodyError.message}`,
            );
          }
        }
        ...
        if (!emailContentBase64) {
          console.warn(
            `Alle Extraktionsmethoden fehlgeschlagen für MIME-Part ${mimePartCounter} - erstelle Platzhalter`,
          );
...
nicht über verfügbare Methoden extrahiert werden konnte.`;
          ...
          emailContentBase64 = btoa(
            unescape(encodeURIComponent(attachmentInfo)),
          );
          console.log(
            `Informativer Platzhalter für MIME-Part ${mimePartCounter} erstellt`,
          );
        }
          ...
          if (part.headers && part.headers["content-disposition"]) {
            for (let disp of part.headers["content-disposition"]) {
              ...
              if (filenameMatch) {
                ...
                console.log(
                  `Dateiname aus filename= extrahiert: ${attachmentName}`,
                );
                break;
              }
              ...
              if (
                filenameMatch &&
                !filenameMatch[1].toLowerCase().includes("attachment") &&
                !filenameMatch[1].toLowerCase().includes("inline")
              ) {
                attachmentName = filenameMatch[1].trim();
                console.log(
                  `Dateiname vom Anfang der Content-Disposition extrahiert: ${attachmentName}`,
                );
                break;
              }
            }
        ...
        if (!attachmentName) {
          ...
          console.log(
            `Generierter Dateiname für unbenannten Part: ${attachmentName}`,
          );
        }
        ...
        if (finalFileName !== fileName) {
          console.log(
            `MIME-Part Dateiname geändert von "${fileName}" zu "${finalFileName}" wegen Duplikat`,
          );
        }

        console.log(
          `CasefolderID für MIME-Part "${finalFileName}": ${selectedCaseFolderID}`,
        );
        ...
        console.log(
          `Beginne Upload für MIME-Part "${finalFileName}" mit ${emailContentBase64.length} Zeichen Base64-Content`,
        );
        ...
        console.log(
          `Upload-Response Status für MIME-Part "${finalFileName}": ${response.status} ${response.statusText}`,
        );

        if (!response.ok) {
          const errorText = await response.text();
          console.error(
            `MIME-Part Upload fehlgeschlagen für "${finalFileName}": ${response.status} - ${errorText}`,
          );
          throw new Error(
            `MIME-Part Upload fehlgeschlagen: ${response.status} - ${errorText}`,
          );
        }
        ...
        console.error(
          `Fehler beim Verarbeiten von MIME-Part "${attachmentName || "unbekannt"}":`,
          error,
        );
        errors.push({
    ...
    if (part.parts && Array.isArray(part.parts)) {
      for (let i = 0; i < part.parts.length; i++) {
        await processMimePart(
          part.parts[i],
          partPath ? `${partPath}.${i}` : `${i}`,
        );
      }
    }
  }
  ...
  // Ergebnis-Summary
  console.log(
    `MIME-Parts Verarbeitung abgeschlossen: ${processedFiles.length} Parts erfolgreich`,
  );
  ...
  if (processedFiles.length > 0) {
    browser.runtime.sendMessage({
      type: "success",
      content: `${processedFiles.length} zusätzliche MIME-Part(s) erfolgreich hochgeladen`,
    });
