...
// Label Nachricht: wird mit dem Betreff der Nachricht gefüllt
browser.runtime.onMessage.addListener((message) => {
    if (message.subject) {
        document.getElementById('messageContent').textContent = message.subject;
        currentMessageToSaveID = message.id;
    }
});


document.addEventListener('DOMContentLoaded', function() {
    let messagesArray = ["Beispiel-Nachricht 1", "Beispiel-Nachricht 2", "Beispiel-Nachricht 3"]; // Beispiel-Array
    let currentIndex = 0;

    // Funktion, um die aktuelle Nachricht anzuzeigen
    function displayMessage(index) {
        document.getElementById("messageContent").textContent = messagesArray[index];
    }
