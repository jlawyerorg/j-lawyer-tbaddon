...
class ImageEditOverlay {
  ...
  initializeElements() {
    ...
    const pdfFilenameSuggestion = document.getElementById(
  ...
  setupEventListeners() {
    ...
    this.elements.cancelBtn.addEventListener("click", () =>
      this.cancelEditing(),
    ); // Finish-Options Abbrechen
    this.elements.cancelBtnNormal.addEventListener("click", () =>
      this.cancelEditing(),
    ); // Normaler Abbrechen
    this.elements.uploadIndividualBtn.addEventListener("click", () =>
      this.finishEditing(false),
    );
    this.elements.createPdfBtn.addEventListener("click", () =>
      this.showPdfRenameDialog(),
    );
    ...
    this.elements.usePdfSuggestionBtn.addEventListener("click", () =>
      this.usePdfSuggestion(),
    );
    this.elements.cancelPdfRenameBtn.addEventListener("click", () =>
      this.hidePdfRenameDialog(),
    );
    this.elements.confirmPdfRenameBtn.addEventListener("click", () =>
    ...
    document.addEventListener("mouseup", () => {
      console.log(
        "GLOBAL: Mouse up - isDragging:",
        this.isDragging,
        "isDraggingCorner:",
        this.isDraggingCorner,
      );
      if (this.isDraggingCorner) {
  ...
  handleInitialDrag(e) {
    ...
    console.log(
      "handleInitialDrag - Current cropBox:",
      currentCropBoxX,
      currentCropBoxY,
    );
    console.log(
      "handleInitialDrag - Start:",
      startX,
      startY,
      "End:",
      endX,
  ...
  handleResizeStart(e) {
    ...
    console.log(
      "handleResizeStart called (deprecated - use onCornerMouseDown)",
    );
    e.stopPropagation();
    e.preventDefault();
  }
  ...
  handleCropBoxMove(e) {
    ...
    console.log(
      "handleCropBoxMove called (deprecated - use onCropBoxMouseDown)",
    );
    e.stopPropagation();
    e.preventDefault();
  }
  ...
  handleCornerDrag(e) {
    ...
    console.log("Corner drag:", handleClass, "new box:", {
      newLeft,
      newTop,
      newWidth,
      newHeight,
  ...
  async finishEditing(createPDF, pdfFileName = null) {
    ...
    try {
      console.log(
        "finishEditing called with createPDF:",
        createPDF,
        "editedImages count:",
        this.editedImages.length,
      );
  ...
  showFinishOptions(imageCount) {
    ...
    const nonImageCount =
      this.nonImageAttachments && this.nonImageAttachments.length
        ? this.nonImageAttachments.length
        : 0;
  ...
  updatePreview() {
    ...
    if (
      ...
      this.elements.previewImage.src.startsWith("blob:")
    ) {
    ...
    console.log(
      ...
      this.editedImages.length,
      "prevDisabled:",
      this.elements.prevImageBtn.disabled,
      "nextDisabled:",
      this.elements.nextImageBtn.disabled,
    );
  }

  showPreviousPreview() {
    console.log(
      "showPreviousPreview called, current index:",
      this.previewIndex,
    );
    if (this.previewIndex > 0 && this.editedImages.length > 1) {
      ...
      console.log(
        "Cannot go to previous image - at first image or only one image",
      );
    }
  }
  ...
  showNextPreview() {
    ...
    if (
      this.previewIndex < this.editedImages.length - 1 &&
      this.editedImages.length > 1
    ) {
      this.previewIndex++;
  ...
  async createAndUploadPDF(customFileName = null) {
    try {
      ...
      for (let i = 0; i < this.editedImages.length; i++) {
        ...
        console.log(
          `Processing image ${i + 1}/${this.editedImages.length}: ${image.name}`,
        );
        ...
        try {
          ...
          console.log(
            "Compressing image for PDF:",
            image.name,
            "original size:",
            image.blob.size,
          );
          const compressedImageData = await this.compressImageForPDF(
            image.blob,
          );
          console.log(
            "Image compressed successfully, new size:",
          ...
          console.log(
            "Compressed image added to PDF successfully:",
            image.name,
          );
        } catch (imageError) {
          console.error(
  ...
  async uploadSingleFile(blob, fileName, contentType) {
    ...
    console.log(
      ...
      url,
      "caseId:",
      this.sessionData.caseData.id,
      "folderId:",
      this.sessionData.selectedCaseFolderID,
    );
    ...
    if (
      result.id &&
      this.sessionData.selectedTags &&
      this.sessionData.selectedTags.length > 0
    ) {
      console.log(
        "Applying tags to document:",
        result.id,
        "tags:",
        this.sessionData.selectedTags,
      );
    ...
    if (
      result.id &&
      this.sessionData.selectedCaseFolderID &&
      this.sessionData.selectedCaseFolderID.length > 0
    ) {
      console.log(
        "Assigning folder to document (additional safety check):",
        result.id,
        "folderId:",
        this.sessionData.selectedCaseFolderID,
      );
      await this.assignFolderToDocument(
        result.id,
        this.sessionData.selectedCaseFolderID,
      );
    }
    ...
    try {
      ...
      if (ocrSettings.performOcr && result.id && this.isOcrEligible(fileName)) {
        console.log("OCR setting is enabled, triggering OCR for:", fileName);
      ...
      console.warn("OCR processing failed (non-critical):", ocrError);
      // OCR-Fehler nicht weiterwerfen, damit Upload nicht komplett fehlschlägt
    }
  }

  async applyTagsToDocument(documentId, tags) {
    console.log(
      "applyTagsToDocument called for document:",
      documentId,
      "with tags:",
      tags,
    );
    ...
    try {
      ...
      for (const tag of tags) {
        ...
        if (!response.ok) {
          const errorText = await response.text();
          console.error(
            "Tag application failed for tag:",
            tag,
            "status:",
            response.status,
            "response:",
          ...
          throw new Error(
            `Failed to apply tag "${tag}": ${response.status} - ${errorText}`,
          );
        }
  ...
  async assignFolderToDocument(documentId, folderId) {
    console.log(
      "assignFolderToDocument called for document:",
      documentId,
      "with folderId:",
      folderId,
    );
    ...
    try {
      const url =
        settings.serverAddress + "/j-lawyer-io/rest/v1/cases/document/update";
      const payload = {
        id: documentId,
        folderId: folderId,
      };
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "Folder assignment failed for document:",
          documentId,
          "status:",
          response.status,
          "response:",
        ...
        throw new Error(
          `Failed to assign folder to document "${documentId}": ${response.status} - ${errorText}`,
        );
      }

      console.log(
        "Folder assigned successfully to document:",
        documentId,
        "folderId:",
        folderId,
      );
  ...
  /**
   * Prüft ob ein Dokument für OCR geeignet ist (anhand der Dateiendung)
   */
  isOcrEligible(fileName) {
    if (!fileName) return false;
    const ocrEligibleExtensions = [
      "pdf",
      "jpg",
      "jpeg",
      "png",
    ...
    return ocrEligibleExtensions.includes(extension);
  }

  /**
   * Führt OCR für ein Dokument durch
   */
  async performOcrOnDocument(documentId, username, password, serverAddress) {
    console.log("Performing OCR on document:", documentId);
    ...
    const url =
      serverAddress +
      "/j-lawyer-io/rest/v6/cases/document/" +
      documentId +
      "/perform-ocr";
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.warn(
          "OCR request failed for document:",
          documentId,
          "status:",
          response.status,
          "response:",
        ...
        // Fehler nur als Warnung loggen, nicht weiterwerfen
        return false;
      }

      console.log("OCR triggered successfully for document:", documentId);
      return true;
    } catch (error) {
      console.warn("Error performing OCR on document:", documentId, error);
      // Fehler nur als Warnung loggen, nicht weiterwerfen
      return false;
    }
  }
